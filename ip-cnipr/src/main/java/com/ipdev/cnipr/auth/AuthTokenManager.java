...
public class AuthTokenManager {
    ...
    public void setLocalDirForTokenCahce(String tokenCacheDir) {
        ...
        tokenLocalFile = localDirForTokenCache + "/" + ACCOUNT_NAME + "-cnipr-token.json";
    }
    ...
    // sample:
    // "https://59.151.93.239/oauth2/access_token?client_id=0F88A0C6E150D4310BA3C9CB61F81929&client_secret=7DF87E7E3AD02A06E950D4ED4E19CE70&redirect_uri=http://221.122.40.156/portal-ps/user/redirect&grant_type=authorization_code&code=534688191c32602a0695f5063f51fc4f";
    ...
    AuthToken requestAccessToken() {
        ...
        /**
         * A valid response looks like: <body> <div id="openid">ab6f1509925e39836a32f7dd22224a15</div> <div
         * id="openkey">8896dbdd8a9323c1e78a9d8c87f5f4c8</div> <div id="expires_in">2592000</div> <div
         * id="access_token">d9eea4e8ad1119ed30b4e8134aa37b47</div> <div id="state">test</div> </body>
         */
    ...
    synchronized void storeTokenToLocalFile(AuthToken authToken) {
        ...
        Path outputPath = Paths.get(tokenLocalFile);
    ...
    synchronized AuthToken readTokenFromLocalFile() {
        if (StringUtils.isEmpty(tokenLocalFile)) {
            return null;
        }
    ...
    public void handleTokenExpireException(AuthTokenExpireException e) {
        // remove the stale Auth token file from local cache
        try {
            Files.deleteIfExists(Paths.get(tokenLocalFile));
        } catch (IOException e1) {
            LOG.error("Caught IOException when trying to delete file: " + tokenLocalFile);
        }
        authToken = null;

        // now, let's try to get access token again
        this.getAuthToken();
    }
